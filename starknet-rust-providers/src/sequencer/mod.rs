...
pub mod models;
use models::{
    AddTransactionResult, Block, BlockId, ContractAddresses, DeployedClass, GatewayBlockTrace,
    GatewayTransactionTrace, StateUpdate, StateUpdateWithBlock, TransactionInfo,
    TransactionRequest, TransactionStatusInfo, conversions::ConversionError,
};
...
impl SequencerGatewayProvider {
        ...
        S: DeserializeOwned,
    {
        ...
        trace!("Sending POST request to sequencer API ({url}): {request_body}");
...
impl SequencerGatewayProvider {
    ...
    ) -> Result<GatewayBlockTrace, ProviderError> {
    ...
    pub async fn get_transaction_trace(
        &self,
        transaction_hash: Felt,
    ) -> Result<GatewayTransactionTrace, ProviderError> {
...
impl From<SequencerError> for ProviderError {
    fn from(value: SequencerError) -> Self {
        ...
        matching_code.map_or_else(
            || GatewayClientError::SequencerError(value).into(),
            Self::StarknetError,
        )
    }
}
