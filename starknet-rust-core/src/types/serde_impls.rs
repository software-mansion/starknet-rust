...
impl<'de> Deserialize<'de> for SyncStatusType {
        ...
        D: Deserializer<'de>,
    {
        match SyncStatusTypeDe::deserialize(deserializer)? {
            SyncStatusTypeDe::Boolean(value) => if value { Err(serde::de::Error::custom("invalid boolean value")) } else { Ok(Self::NotSyncing) },
...
mod enum_ser_impls {
    use super::super::{Serialize, Transaction, TransactionContent, BroadcastedTransaction, InvokeTransaction, InvokeTransactionContent, DeclareTransaction, DeclareTransactionContent, DeployAccountTransaction, DeployAccountTransactionContent, TransactionReceipt, TransactionTrace, ExecuteInvocation};
